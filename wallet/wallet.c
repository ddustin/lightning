...
void wallet_inflight_add(struct wallet *w, struct channel_inflight *inflight)
{
	...
	log_debug(w->log, "Adding inflight with output index %d and txid %s",
		  (int)inflight->funding->outpoint.n,
		  type_to_string(tmpctx, struct bitcoin_txid, &inflight->funding->outpoint.txid));
